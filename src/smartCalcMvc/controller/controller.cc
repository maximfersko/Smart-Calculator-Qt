...
std::pair<std::vector<double>, std::vector<double>> controller::graphBuilder(
    std::vector<double> &Data, std::string str) {
    return model_->graphBuilder(Data, str);
}

typename controller::data_t_credit controller::Annuity(double summa,
                                                       double period,
                                                       double rate) {
...
typename controller::data_t_credit controller::deffirentated(double summa,
                                                             double period,
                                                             double rate) {
...
void controller::SetParam(double amount, double period, double rate,
                               bool capitalization, double taxRate,
                               int replCount, double replAmount,
                               int withdrCount, double withdrAmount) {
                                depositModel_->calculateDeposit(amount, period, rate, capitalization, taxRate, replCount, replAmount, withdrCount, withdrAmount);
                               }
...
double controller::getTax() {
    return depositModel_->getTax();
}

double controller::getInterestDeposit() {
    return depositModel_->getInterest();
}
