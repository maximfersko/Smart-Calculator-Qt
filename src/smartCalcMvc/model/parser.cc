...
int SmartCalc::GetTypeFunction(std::string &str, int type) {
    ...
    switch (type) {
        case COS:
            if (sub.substr(0, 3) == "cos") token = COS;
            break;
        case TAN:
            if (sub.substr(0, 3) == "tan") token = TAN;
            break;
        case SIN:
        ...
        if (it != elements_.end()) {
            ...
            if (result == DIV || result == MULT || result == MOD) {
                ...
                *flag = false;
                ...
                if (result == UNDEFUIEND) Error("invalid expressions !");
            }
        }
    }
    ...
    if (operations_.empty() ||
        ...
        while (!operations_.empty()) {
            if (GetPriority(operations_.back().first) >= GetPriority(token)) {
                numbers_.push_back(
                    std::make_pair(operations_.back().first, OPERATION));
